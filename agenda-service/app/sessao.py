...
precisa abrir uma sessão própria para resolver a credencial no banco, e
`db.py` importa `auth.py` (para a dependência `get_db`). Manter a sessão
aqui quebra o ciclo.
"""
...
def sessao_worker(db: Session) -> None:
    """Modo worker (jobs e lookup de credencial): a política RLS libera todas
    as orgs para este GUC — só o backend consegue setá-lo (equivalente
    explícito do service_role).

    Nunca chame isto na sessão de uma requisição: o GUC vale para a transação
    inteira, então dali em diante TODA query daquela transação cruza orgs.
    Abra uma sessão própria e efêmera, como `auth._resolver_credencial`.
    """
    db.info["worker"] = True
